...
- `[security.service_tokens].refresh_grace_seconds` (Default 3600 = 1h) definiert ein Zeitfenster, in dem abgelaufene Tokens **ausschließlich** über den Token-Exchange-Endpoint (`/modules/runtime/tokens`) noch einen frischen Token erhalten können. Reguläre Gateway-/Connector-Calls (`validate_service_token`) bleiben strikt – abgelaufene Tokens werden dort sofort abgelehnt. Der Exchange-Handler nutzt `SecurityManager::validate_service_token_for_refresh`, der `ServiceTokenStore::validate_for_refresh` delegiert. Damit recovern Module nach System-Sleep (macOS Lid Close, Suspend) automatisch ohne manuellen Restart. Grace-Refreshes erzeugen Audit-Events mit `reason = grace_period_refresh`. `cleanup_if_needed` berücksichtigt die Grace-Period und entfernt Tokens erst nach Ablauf des erweiterten Fensters.
...
- Control-Plane-Tokens werden nie im Repo hinterlegt – lokale Automatisierung zieht sie aus sicheren Stores (macOS Keychain via `security find-generic-password`, Vault, o. Ä.) und exportiert sie runtime-only als `FENRIR_CONTROL_TOKEN`/`FENRIR_HTTP_TOKEN_ADMIN`, damit `fenrirctl`/Scripts HTTP-Aufrufe autorisieren können.
  ...
  - `[modules.service_profiles."<profile>"]` definiert wiederverwendbare Policies für Service-Descriptoren (`internal_only`, `allowed_roles`, `required_scopes`, `tenant`, optional `ingress_access`, `rate_limit_per_second` oder `disable_rate_limit`). Module können im `/.fenrir/services` einen `profile` angeben; Profile werden vor den per-Service-Overrides angewendet.
...
- Module deklarieren produktive Services über einen HTTP-Descriptor (`/.fenrir/services`). Nutzt `fenrir-module-kit::service` für den Payload. Module können den Descriptor zusätzlich per Control-Plane pushen (`POST /modules/runtime/services`); der Payload ist HMAC-signiert (Token-basiert) und enthält `schema_version`, `signed_at` und `signature`. Fenrir registriert `module:<id>::service` im ServiceRegistry und schreibt die Zuordnung nach `runtime/services.json`, welches jeder Modulprozess via `FENRIR_SERVICE_SNAPSHOT_PATH` konsumiert. `.fenrir/config.toml`-`[[services]]` bleibt ausschließlich für Dev-Overrides (`sync module`) bzw. Stub-Runtime gedacht.
...
- Öffentliche Statusabfragen laufen über `GET /public/status` (ohne Control-Plane-Token). Der Endpoint liefert ausschließlich ein sanitisiertes Snapshot (`overall`, `updated_at`, `live`, `ready`) plus kuratierte `components[]` (`website|account|api`, optional `notifications`) sowie `incidents[]` (open/resolved, severity, timestamps). Keine Service-IDs, Security-/Ingress-Details oder Rohdiagnostik im Public-Payload. Die Komponentensicht basiert auf expliziten Modul-/Service-Mappings und respektiert Public-Visibility (`ingress.access = public`) als Sicherheits-Gate.
...
- Readiness-Probe `services` bewertet nur noch **kritische** Registry-Einträge (`descriptor.critical`) als Blocking-Condition; fehlgeschlagene Auxiliary-/Placeholder-Services degradieren Observability, blockieren aber `/ready` nicht mehr.
