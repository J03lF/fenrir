...
use super::context::{SqlContext, Expecting};
...
impl SqlCompletionEngine {
    ...
    pub fn update_schema(&mut self, tables: HashSet<String>, columns: HashMap<String, Vec<String>>) {
    ...
    fn generate_suggestions(
        ...
        match context.expecting {
            // Statement start
            Expecting::Statement => {
                self.filter_keywords(keywords::STATEMENT_KEYWORDS, prefix)
            }
            ...
            Expecting::FromKeyword => {
                self.filter_keywords(&["FROM"], prefix)
            }
            
            // INTO keyword required (after INSERT)
            Expecting::IntoKeyword => {
                self.filter_keywords(&["INTO"], prefix)
            }
            
            // Table name expected
            Expecting::TableName => {
                self.schema.tables_with_prefix(prefix)
            }
            
            // Column name expected
            Expecting::ColumnName => {
                self.columns_for_context(context, prefix)
            }
            ...
            // DDL object type
            Expecting::DdlObject => {
                self.filter_keywords(keywords::CREATE_OBJECTS, prefix)
            }
            ...
            Expecting::JoinCondition => {
                ...
                for table in &context.tables {
                    ...
                    let qualifier = context.aliases.iter()
                        .find(|(_, t)| *t == table)
                        .map(|(a, _)| a.as_str())
                    ...
                    if let Some(cols) = self.schema.columns_for(table) {
                        for col in cols {
                            ...
                            if prefix.is_empty() || qualified.to_uppercase().starts_with(&prefix.to_uppercase()) {
                                sugg.push(qualified);
                            }
                        }
                    }
    ...
    fn qualified_columns_for_context(
        ...
        if let Some(q) = qualifier {
            ...
            let table_name = context.resolve_alias(q)
                .map(|s| s.as_str())
                .unwrap_or(q);
...
impl CompletionResult {
    ...
    pub fn contains(&self, suggestion: &str) -> bool {
        self.suggestions.iter().any(|s| s.eq_ignore_ascii_case(suggestion))
    }
...
mod tests {
    ...
    fn test_engine() -> SqlCompletionEngine {
        ...
        columns.insert("users".to_string(), vec!["id".into(), "name".into(), "email".into()]);
        columns.insert("tickets".to_string(), vec!["id".into(), "user_id".into(), "title".into(), "status".into()]);
        columns.insert("comments".to_string(), vec!["id".into(), "ticket_id".into(), "content".into()]);
