...
pub const SELECT_MODIFIERS: &[&str] = &[
    "DISTINCT", "ALL",
];
...
pub const NULL_KEYWORDS: &[&str] = &[
    "NULL", "NOT NULL", "IS NULL", "IS NOT NULL",
];

/// ORDER BY modifiers
pub const ORDER_MODIFIERS: &[&str] = &[
...
pub const AFTER_INSERT_TABLE: &[&str] = &[
    "VALUES", "SELECT", "DEFAULT",
];

/// Keywords for after SET column = value
pub const AFTER_SET_VALUE: &[&str] = &[
    "WHERE",
];

/// DDL: CREATE object types
pub const CREATE_OBJECTS: &[&str] = &[
    "TABLE", "INDEX", "UNIQUE", "VIEW", "MATERIALIZED",
    "SCHEMA", "DATABASE", "SEQUENCE",
    "FUNCTION", "PROCEDURE", "TRIGGER",
    "TYPE", "EXTENSION", "ROLE", "USER",
];

/// DDL: DROP object types
pub const DROP_OBJECTS: &[&str] = &[
    "TABLE", "INDEX", "VIEW", "MATERIALIZED",
    "SCHEMA", "DATABASE", "SEQUENCE",
    "FUNCTION", "PROCEDURE", "TRIGGER",
    "TYPE", "EXTENSION", "ROLE", "USER",
    "IF",  // IF EXISTS
...
pub const ALTER_ACTIONS: &[&str] = &[
    "ADD", "DROP", "ALTER", "RENAME", "SET", "RESET",
    "COLUMN", "CONSTRAINT", "INDEX", "OWNER", "SCHEMA",
];
...
pub fn all_keywords() -> HashSet<String> {
    ...
    for &kw in STATEMENT_KEYWORDS { set.insert(kw.to_string()); }
    for &kw in SELECT_MODIFIERS { set.insert(kw.to_string()); }
    for &kw in AGGREGATE_FUNCTIONS { set.insert(kw.to_string()); }
    for &kw in SCALAR_FUNCTIONS { set.insert(kw.to_string()); }
    for &kw in AFTER_SELECT_COLUMN { set.insert(kw.to_string()); }
    for &kw in AFTER_FROM_TABLE { set.insert(kw.to_string()); }
    for &kw in JOIN_TYPES { set.insert(kw.to_string()); }
    for &kw in COMPARISON_OPERATORS { set.insert(kw.to_string()); }
    for &kw in LOGICAL_OPERATORS { set.insert(kw.to_string()); }
    for &kw in NULL_KEYWORDS { set.insert(kw.to_string()); }
    for &kw in ORDER_MODIFIERS { set.insert(kw.to_string()); }
    for &kw in CREATE_OBJECTS { set.insert(kw.to_string()); }
    for &kw in DROP_OBJECTS { set.insert(kw.to_string()); }
    for &kw in ALTER_ACTIONS { set.insert(kw.to_string()); }
    for &kw in DATA_TYPES { set.insert(kw.to_string()); }
